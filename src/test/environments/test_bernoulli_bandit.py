...
class TestBernoulliBandit:
    def test_draw(self):
        with_certainty = BernoulliBandit(init_means=[0.0, 1.0], horizon=2, rng=default_rng())
        assert with_certainty.draw(t=0, action=0) == 0
        assert with_certainty.draw(t=1, action=1) == 1

        env = BernoulliBandit(init_means=[0.5], horizon=2, rng=Generator(PCG64(42)))
        assert env.draw(t=0, action=0) == 1
        assert env.draw(t=0, action=0) == 0

    def test_means_tracking(self):
        env = BernoulliBandit(init_means=[0.4, 0.6], horizon=4, rng=default_rng())
